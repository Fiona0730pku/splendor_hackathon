...
class NeuralNetAI(AI):
    ...
    def __init__(self, *args, num_players=3, stepsize=0.05, restore=False, prob_factor=1., **kwargs):
    ...
    def train(self, training_data, stepsize_multiplier=1., stepsize=0.01):
        for winner_index, state_vectors in training_data:
            ...
            expected_output = np.zeros(num_players)
            expected_output[winner_index] = num_players-1
            for _ in range(10):
                try:
...
class H50AI(NeuralNetAI):
    ...
    def make_graph(self):
        INPUT_SIZE = 3277 #2300 #2192  # 2170 # 1022 #987 #935 #986 #818 #767 #647 #479 #647 #563 #479 #395 #647 # 395 #407 #297 #345 #249 #265 # 305 # 265 # 585
        # INPUT_SIZE = 293 # 294 # 613
        HIDDEN_LAYER_SIZE = [60, 30]
        ...
        weight_1 = tf.Variable(tf.truncated_normal([INPUT_SIZE, HIDDEN_LAYER_SIZE[0]], stddev=0.1),
                               name='weight_1')
        bias_1 = tf.Variable(tf.truncated_normal([HIDDEN_LAYER_SIZE[0]], stddev=0.1),
                             name='bias_1')
        ...
        # Hidden layers
        inter_weight = tf.Variable(tf.truncated_normal([HIDDEN_LAYER_SIZE[0], HIDDEN_LAYER_SIZE[1]], stddev=0.1),
                                name='inter_weight_1')
        inter_bias = tf.Variable(tf.truncated_normal([HIDDEN_LAYER_SIZE[1]], stddev=0.1),
                                name='inter_bias_1')

        inter_output_2 = tf.nn.relu(tf.matmul(hidden_output_1, inter_weight) + inter_bias)
        hidden_output_2 = tf.nn.dropout(inter_output_2, 0.80)

        # Output layer
        weight_2 = tf.Variable(tf.truncated_normal([HIDDEN_LAYER_SIZE[1], 3], stddev=0.1),
                               name='weight_2')
        bias_2 = tf.Variable(tf.truncated_normal([3], stddev=0.1),
                             name='bias_2')
        ...
        output = tf.matmul(hidden_output_2, weight_2) + bias_2
        softmax_output = tf.nn.softmax(output)

        real_result = tf.placeholder(tf.float32, [None, 3], name='real_result')
...
class H50AI_TDlam(H50AI):
    ...
    def get_output(self, input_vec):
        hidden_output_1 = tf.nn.relu(tf.matmul(input_vec, self.weight_1) + self.bias_1)
        inter_output_2 = tf.nn.relu(tf.matmul(hidden_output_1, self.inter_weight) + self.inter_bias)
        hidden_output_2 = tf.nn.dropout(inter_output_2, 0.80)
        output = tf.matmul(hidden_output_2, self.weight_2) + self.bias_2
        softmax_output = tf.nn.softmax(output)
        ...
        for row_index, row in enumerate(training_data):
            ...
            for i, v in enumerate(state_vectors):
                ...
                pre_move_grads = [self.session.run(self.grads, feed_dict={
                    self.input_state: vec.reshape([1, -1]),
                    self.real_result: real_result.reshape([1, 3])}) for vec, real_result in zip(move_info.pre_move_vecs,
                                                                                move_info.post_move_values)]
            ...
            for player_index in range(self.num_players):
                ...
                self.session.run(self.train_step, feed_dict={
                    ...
                    self.real_result: last_move_info.post_move_values[player_index].reshape((-1, 3)),
                    self.stepsize_multiplier: stepsize_multiplier,
    ...
    def train(self, training_data, stepsize_multiplier=1., stepsize=0.01):
        ...
        for row_index, row in enumerate(training_data):
            ...
            for i, v in enumerate(state_vectors):
                ...
                for player_index in range(self.num_players):
                    ...
                    for ni, nv in enumerate(state_vectors[i:]):
                        ...
                        try:
                            post_move_value = post_move_value.reshape((-1, 3))
            ...
            for i, v in enumerate(state_vectors):
                ...
                assert len(input_states) == 3  # This code currently only works properly with 2 players
            ...
            if row_index == 0:
                print('\nExample game:')
                for i, move_info in enumerate(state_vectors):
                    print(i % 3, move_info.move, move_info.post_move_values[0])
                print()
        print()
